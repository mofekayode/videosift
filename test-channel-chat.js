...
async function testChannelChat() {
  ...
  // Test channel chat endpoint
  try {
    const response = await fetch('http://localhost:3001/api/chat-channel-stream', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        channelId: '13f079a2-1446-46a7-adba-3a9660e36765', // Exponent channel
        message: 'What interview strategies are discussed in the videos?',
        sessionId: null
      })
    });
